...
from config import AUTO_LOCK_SECONDS, RATE_LIMIT_REQUESTS
...
def is_rate_limited(user_id: int) -> bool:
    ...
    if len(_rate_buckets[user_id]) >= RATE_LIMIT_REQUESTS:
        return True
...
# {user_id: {"last_activity": float, "locked": bool, "key": bytes|None}}
_sessions: dict[int, dict] = {}


def touch(user_id: int):
    if user_id not in _sessions:
        _sessions[user_id] = {"last_activity": time.time(), "locked": False, "key": None}
    else:
        _sessions[user_id]["last_activity"] = time.time()


def set_key(user_id: int, key: bytes):
    touch(user_id)
    _sessions[user_id]["key"] = key
    _sessions[user_id]["locked"] = False
...
def get_key(user_id: int):
    ...
    if s.get("locked"):
        return None
    if time.time() - s.get("last_activity", 0) > AUTO_LOCK_SECONDS:
        s["locked"] = True
        s["key"] = None
        return None
    return s.get("key")
...
def is_locked(user_id: int) -> bool:
    ...
    if time.time() - s.get("last_activity", 0) > AUTO_LOCK_SECONDS:
        ...
        s["key"] = None
        return True
    return s.get("locked", True)


def lock(user_id: int):
    if user_id in _sessions:
        _sessions[user_id]["locked"] = True
        _sessions[user_id]["key"] = None


def unlock(user_id: int, key: bytes):
    set_key(user_id, key)
...
def generate_password(length: int = 16) -> str:
    ...
    while True:
        ...
        # Минимальные требования: буква, цифра, спецсимвол
...
# 2048 слов BIP39 (английский список)
BIP39_WORDS = [
    ...
    "boss","bottom","bounce","boy","bracket","brain","brand","brave",
    ...
    "equal","equip","erase","erode","erosion","error","erupt","escape",
    "essay","essence","estate","eternal","ethics","evidence","evil","evoke",
    "evolve","exact","example","excess","exchange","excite","exclude","exercise",
    "exhaust","exhibit","exile","exist","exit","exotic","expand","expire",
    "explain","expose","express","extend","extra","eye","fable","face",
    "faculty","faint","faith","fall","false","fame","family","famous",
    "fan","fancy","fantasy","far","fashion","fat","fatal","father",
    "fatigue","fault","favorite","feature","february","federal","fee","feed",
    "feel","feet","fellow","felt","fence","festival","fetch","fever",
    ...
    "guitar","gun","gym","habit","hair","half","hamster","hand",
    ...
    "knife","knock","know","lab","ladder","lake","lamp","language",
    ...
    "master","match","material","math","maze","meadow","mean","medal",
    ...
    "miracle","miss","mitten","model","modify","mom","monitor","monkey",
    ...
    "myself","mystery","naive","name","napkin","narrow","nasty","natural",
    ...
    "robot","robust","rocket","romance","roof","rookie","rose","rotate",
    "rough","royal","rubber","rude","rug","rule","run","runway",
    "rural","sad","saddle","sadness","safe","sail","salad","salmon",
    "salon","salt","salute","same","sample","sand","satisfy","satoshi",
    "sauce","sausage","save","say","scale","scan","scatter","scene",
    ...
    "unique","universe","unknown","unlock","until","unusual","unveil","update",
...
def generate_seed_phrase(words: int = 12) -> str:
    """BIP39-совместимая seed-фраза (12 или 24 слова)."""
    chosen = [secrets.choice(BIP39_WORDS) for _ in range(words)]
    return " ".join(chosen)
...
def detect_lang(locale_code: str) -> str:
    """Определяет язык по Telegram locale."""
    if locale_code and locale_code.startswith("ru"):
        return "ru"
    return "en"
